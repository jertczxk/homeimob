...
import {
  ...
  Calendar, Tag, FileText, Clock, Globe,
} from 'lucide-react'
...
export default function BlogListPage() {
  ...
  const stats = {
    ...
    publicado: mockBlogPosts.filter(p => p.status === 'publicado').length,
    rascunho: mockBlogPosts.filter(p => p.status === 'rascunho').length,
  }
          ...
          {(['todos', 'publicado', 'rascunho', 'agendado'] as const).map(s => (
            <button
              ...
              className={cn(
                ...
                filterStatus === s ? 'bg-accent/15 text-accent' : 'text-zinc-500 hover:text-white'
              )}
            >
              {s}
            </button>
              ...
              >
                <div className="flex gap-5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        {post.resumo && (
                          <p className="text-xs text-zinc-500 mt-1 line-clamp-1">{post.resumo}</p>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                    ...
                    <button className="w-8 h-8 flex items-center justify-center rounded-lg text-zinc-500 hover:text-red-400 hover:bg-red-500/10 transition-colors">
