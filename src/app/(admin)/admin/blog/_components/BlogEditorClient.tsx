...
export function BlogEditorClient({ post }: { post: BlogPost }) {
  ...
  const [postStatus, setPostStatus] = useState<string>(post.status)
  ...
  function handleSave() {
    ...
    formData.set('status', postStatus)
    formData.set('autor', post.autor)
    startTransition(() => updateBlogPost(post.id, formData))
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleDelete} disabled={isPending} className="flex items-center gap-2 bg-red-500/10 text-red-400 px-3 py-2 rounded-lg text-sm hover:bg-red-500/20 transition-colors border border-red-500/10 disabled:opacity-40">
            <Trash2 className="h-4 w-4" />
          </button>
          <select value={postStatus} onChange={(e) => setPostStatus(e.target.value)} className="bg-zinc-800/50 border border-white/5 rounded-lg px-3 py-2 text-sm text-zinc-300 outline-none cursor-pointer">
            <option value="rascunho">Rascunho</option>
            <option value="publicado">Publicado</option>
          </select>
          <button onClick={handleSave} disabled={isPending} className="flex items-center gap-2 bg-accent text-accent-foreground px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-accent/90 transition-colors disabled:opacity-50">
