# Seed Mock Data — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Popular as 13 tabelas do Supabase com dados mock realistas e coerentes para o painel admin da HOME Imob.

**Architecture:** SQL puro executado via `mcp__supabase__execute_sql`. UUIDs fixos hardcoded para garantir referências FK entre tabelas. Imoveis usam `ON CONFLICT (slug) DO NOTHING`. Todas as demais tabelas assumidas vazias. Datas cobrem out/2025–abr/2026.

**Tech Stack:** PostgreSQL, Supabase MCP (`mcp__supabase__execute_sql`), project_id: `oqmaymqoifyfhrrcyyim`

---

### UUID Reference

Usado em todo o plano para manter consistência:

```
-- Clientes
cli-01: '00000000-0000-0000-0001-000000000001'  -- Rodrigo Schaefer (proprietário)
cli-02: '00000000-0000-0000-0001-000000000002'  -- Fernanda Wachholz (proprietária)
cli-03: '00000000-0000-0000-0001-000000000003'  -- Carlos Meurer (proprietário)
cli-04: '00000000-0000-0000-0001-000000000004'  -- Beatriz Zimmermann (proprietária)
cli-05: '00000000-0000-0000-0001-000000000005'  -- André Becker (proprietário)
cli-06: '00000000-0000-0000-0001-000000000006'  -- Patrícia Lange (inquilina)
cli-07: '00000000-0000-0000-0001-000000000007'  -- Lucas Reinhardt (inquilino)
cli-08: '00000000-0000-0000-0001-000000000008'  -- Mariana Furtado (inquilina)
cli-09: '00000000-0000-0000-0001-000000000009'  -- Gabriel Oliveira (inquilino)
cli-10: '00000000-0000-0000-0001-000000000010'  -- Camila Ramos (inquilina)
cli-11: '00000000-0000-0000-0001-000000000011'  -- Felipe Teixeira (comprador)
cli-12: '00000000-0000-0000-0001-000000000012'  -- Juliana Soares (compradora)
cli-13: '00000000-0000-0000-0001-000000000013'  -- Henrique Borges (proprietário/vendedor)
cli-14: '00000000-0000-0000-0001-000000000014'  -- Construtora Litoral SC Ltda (PJ)
cli-15: '00000000-0000-0000-0001-000000000015'  -- Farmácias Rede Sul Ltda (PJ)

-- Leads
led-01..12: '00000000-0000-0000-0002-0000000000XX' (XX = 01..12)

-- Contratos
ctr-01..05: '00000000-0000-0000-0003-0000000000XX' (XX = 01..05)

-- Pagamentos
pag-01..25: '00000000-0000-0000-0004-0000000000XX' (XX = 01..25)

-- Repasses
rep-01..19: '00000000-0000-0000-0005-0000000000XX' (XX = 01..19)

-- Vistorias
vis-01..08: '00000000-0000-0000-0006-0000000000XX' (XX = 01..08)

-- Manutencoes
man-01..08: '00000000-0000-0000-0007-0000000000XX' (XX = 01..08)

-- Propostas
pro-01..04: '00000000-0000-0000-0008-0000000000XX' (XX = 01..04)

-- Visitas Vendas
vvd-01..06: '00000000-0000-0000-0009-0000000000XX' (XX = 01..06)

-- Captacoes
cap-01..08: '00000000-0000-0000-000a-0000000000XX' (XX = 01..08)

-- Blog Posts
blg-01..03: '00000000-0000-0000-000b-0000000000XX' (XX = 01..03)
```

---

### Task 1: Verificar conectividade e estado inicial

**Files:** nenhum (SQL direto)

- [ ] **Step 1: Verificar tabelas existentes**
...
Esperado: lista com pelo menos `blog_posts, captacoes, clientes, contratos, imoveis, imovel_fotos, lead_interactions, leads, manutencoes, pagamentos, propostas, repasses, visitas_vendas, vistorias`.

- [ ] **Step 2: Verificar contagens atuais**

```sql
SELECT
  'clientes' AS tabela, count(*) FROM public.clientes
...
Anotar os valores. Se `imoveis > 0`, o seed anterior já rodou — o ON CONFLICT vai preservar esses registros.

---

### Task 2: Seed clientes (15 registros)

**Files:** nenhum

- [ ] **Step 1: Inserir os 15 clientes**
...
```sql
SELECT count(*) AS total FROM public.clientes;
```
...
### Task 3: Seed imoveis (30 registros)

**Files:** nenhum

- [ ] **Step 1: Inserir 30 imóveis**
...
```sql
SELECT count(*) AS total, finalidade, tipo FROM public.imoveis GROUP BY finalidade, tipo ORDER BY finalidade, tipo;
```

Esperado: `venda/residencial=14, venda/comercial=4, locação/residencial=10, locação/comercial=2`

---

### Task 4: Seed leads (12) + lead_interactions (24)

**Files:** nenhum

- [ ] **Step 1: Inserir 12 leads**
...
- [ ] **Step 2: Inserir 24 interações (2 por lead)**
...
Esperado: `leads=12, lead_interactions=24`

---

### Task 5: Seed contratos (5)

**Files:** nenhum

- [ ] **Step 1: Inserir 5 contratos**
...
(
  ...
  3200, '2025-11-01', '2027-10-31', 10, 8.0, 'ativo',
  '2025-11-01', NOW()
),
(
  ...
  2100, '2025-10-01', '2027-09-30', 5, 8.0, 'ativo',
  '2025-10-01', NOW()
),
(
  ...
  4800, '2025-12-01', '2027-11-30', 10, 8.0, 'ativo',
  '2025-12-01', NOW()
),
(
  ...
  2400, '2026-01-01', '2027-12-31', 10, 8.0, 'ativo',
  '2026-01-01', NOW()
),
(
  ...
  2000, '2026-02-01', '2028-01-31', 10, 8.0, 'ativo',
  '2026-02-01', NOW()
);
```
...
```sql
SELECT c.id, i.titulo, prop.nome AS proprietario, inq.nome AS inquilino, c.valor_aluguel
FROM public.contratos c
JOIN public.imoveis i ON i.id = c.imovel_id
JOIN public.clientes prop ON prop.id = c.proprietario_id
JOIN public.clientes inq ON inq.id = c.inquilino_id;
```

Esperado: 5 linhas com dados coerentes.

---

### Task 6: Seed pagamentos (25)

**Files:** nenhum

- [ ] **Step 1: Inserir pagamentos do ctr-01 (nov/2025–abr/2026, 6 registros)**
...
- [ ] **Step 2: Inserir pagamentos do ctr-02 (out/2025–abr/2026, 7 registros)**
...
- [ ] **Step 3: Inserir pagamentos do ctr-03 (dez/2025–abr/2026, 5 registros — com 1 atrasado)**
...
- [ ] **Step 4: Inserir pagamentos do ctr-04 (jan/2026–abr/2026, 4 registros)**
...
- [ ] **Step 5: Inserir pagamentos do ctr-05 (fev/2026–abr/2026, 3 registros)**
...
```sql
SELECT status, count(*) FROM public.pagamentos GROUP BY status;
```

Esperado: `pago=19, pendente=5, atrasado=1` (total=25)

---

### Task 7: Seed repasses (19)

**Files:** nenhum

- [ ] **Step 1: Inserir os 19 repasses (apenas pagamentos com status='pago')**
...
-- ctr-01: 5 repasses (nov/2025–mar/2026)
...
-- ctr-02: 6 repasses (out/2025–mar/2026)
...
-- ctr-03: 3 repasses (dez/2025–fev/2026 — mar/2026 está atrasado)
...
-- ctr-04: 3 repasses (jan/2026–mar/2026)
...
-- ctr-05: 2 repasses (fev/2026–mar/2026)
...
```sql
SELECT count(*) AS total, sum(valor_liquido) AS total_repassado FROM public.repasses;
```

Esperado: `total=19, total_repassado=49864`

---

### Task 8: Seed vistorias (8) e manutencoes (8)

**Files:** nenhum

- [ ] **Step 1: Inserir 8 vistorias**

```sql
INSERT INTO public.vistorias (id, contrato_id, tipo, data, laudo, created_at) VALUES
-- Entradas (uma por contrato)
...
('00000000-0000-0000-0007-000000000001',
  (SELECT id FROM public.imoveis WHERE slug = 'apartamento-trindade-florianopolis'),
  'Infiltração no teto do banheiro','Inquilina relatou mancha de umidade no teto do banheiro social após chuvas fortes. Necessário inspeção no apartamento do andar superior.','aberta',NULL,'2026-02-16',NULL,'2026-02-16'),
('00000000-0000-0000-0007-000000000002',
  (SELECT id FROM public.imoveis WHERE slug = 'studio-itacorubi-florianopolis'),
  'Torneira da pia com vazamento','Inquilino reportou gotejamento na torneira da pia do banheiro. Aperto de vedação ou troca de peça necessária.','aberta',NULL,'2026-03-20',NULL,'2026-03-20'),
('00000000-0000-0000-0007-000000000003',
  (SELECT id FROM public.imoveis WHERE slug = 'casa-ingleses-florianopolis'),
  'Pintura interna — sala e quartos','Inquilina solicitou repintura das paredes da sala e dos 3 quartos. Tintas claras com manchas de umidade do inverno passado.','em_andamento',1800,'2026-03-05',NULL,'2026-03-05'),
('00000000-0000-0000-0007-000000000004',
  (SELECT id FROM public.imoveis WHERE slug = 'apartamento-trindade-florianopolis'),
  'Reparo elétrico — disjuntor da cozinha','Disjuntor da cozinha desarmando com frequência. Eletricista contratado para inspeção e substituição do quadro.','em_andamento',650,'2026-04-02',NULL,'2026-04-02'),
('00000000-0000-0000-0007-000000000005',
  (SELECT id FROM public.imoveis WHERE slug = 'apartamento-centro-joinville-locacao'),
  'Troca de fechadura — porta principal','Inquilino perdeu chave reserva. Solicitada troca de cilindro e confecção de 3 novas chaves.','em_andamento',280,'2026-04-05',NULL,'2026-04-05'),
('00000000-0000-0000-0007-000000000006',
  (SELECT id FROM public.imoveis WHERE slug = 'casa-ingleses-florianopolis'),
  'Revisão hidráulica completa','Revisão preventiva de toda a rede hidráulica antes da renovação do contrato. Substituição de sifões e registros.','concluida',920,'2025-12-10','2025-12-15','2025-12-10'),
('00000000-0000-0000-0007-000000000007',
  (SELECT id FROM public.imoveis WHERE slug = 'studio-itacorubi-florianopolis'),
  'Instalação de ar-condicionado split','Proprietária solicitou instalação de split 9000 BTUs no dormitório para valorizar o imóvel.','concluida',2400,'2026-01-08','2026-01-10','2026-01-08'),
('00000000-0000-0000-0007-000000000008',
  (SELECT id FROM public.imoveis WHERE slug = 'apartamento-cordeiros-itajai-locacao'),
  'Troca de revestimento do banheiro','Azulejos antigos com trincas na área do box. Substituição completa do revestimento do banheiro.','concluida',3200,'2025-12-20','2026-01-05','2025-12-20');
```

- [ ] **Step 3: Verificar**

```sql
SELECT count(*) FROM public.vistorias;
SELECT status, count(*) FROM public.manutencoes GROUP BY status;
```

Esperado: `vistorias=8`, `manutencoes: aberta=2, em_andamento=3, concluida=3`

---

### Task 9: Seed propostas (4) e visitas_vendas (6)

**Files:** nenhum

- [ ] **Step 1: Inserir 4 propostas**
...
(
  ...
  'Felipe Teixeira',
  'Henrique Borges',
  750000, 720000, 740000, '2026-04-01', '2026-04-15', 'contraproposta', 'Marco Vieira', NOW()-INTERVAL '13 days', NOW()-INTERVAL '5 days'
),
(
  ...
  'Juliana Soares',
  'Fernanda Wachholz',
  1200000, 1140000, NULL, '2026-03-28', '2026-04-28', 'aguardando', 'Ana Lima', NOW()-INTERVAL '17 days', NOW()-INTERVAL '2 days'
),
(
  ...
  'Juliana Soares',
  'Construtora Litoral SC Ltda',
  950000, 920000, NULL, '2026-02-15', '2026-03-15', 'aceita', 'Ana Lima', NOW()-INTERVAL '58 days', NOW()-INTERVAL '30 days'
),
(
  ...
  'Felipe Teixeira',
  'Rodrigo Schaefer',
  780000, 700000, NULL, '2026-01-10', '2026-01-25', 'recusada', 'Marco Vieira', NOW()-INTERVAL '94 days', NOW()-INTERVAL '80 days'
);
```

- [ ] **Step 2: Inserir 6 visitas de vendas**
...
(
  ...
  '2026-03-22 10:00:00', 'realizada', 'Ana Lima',
  'Visita de 1h. Elena adorou o terraço e a vista. Pediu simulação de financiamento.',
  '2026-03-18'
),
(
  ...
  '2026-03-15 14:30:00', 'realizada', 'Ana Lima',
  'Igor veio com a esposa. Visitaram 2 unidades no condomínio. Iniciou negociação.',
  '2026-03-10'
),
(
  ...
  (SELECT id FROM public.imoveis WHERE slug = 'casa-america-joinville'),
  '2026-03-28 09:00:00', 'realizada', 'Marco Vieira',
  'Henrique trouxe o pai para avaliar. Muito positivo. Proposta elaborada na semana seguinte.',
  '2026-03-25'
),
(
  ...
  '2026-04-18 11:00:00', 'agendada', 'Ana Lima',
  'Carolina confirmou presença. Vem de Joinville especialmente para ver o imóvel.',
  '2026-04-12'
),
(
  ...
  (SELECT id FROM public.imoveis WHERE slug = 'apartamento-nacoes-balneario-camboriu'),
  '2026-04-20 15:00:00', 'agendada', 'Marco Vieira',
  'Diego quer ver pessoalmente antes de fechar. Será a segunda visita.',
  '2026-04-14'
),
(
  ...
  '2026-03-05 10:00:00', 'cancelada', 'Marco Vieira',
  'Lead cancelou no dia anterior. Pediu reagendamento mas não retornou.',
  '2026-03-01'
);
```
...
```sql
SELECT status, count(*) FROM public.propostas GROUP BY status;
SELECT status, count(*) FROM public.visitas_vendas GROUP BY status;
```

Esperado: `propostas: contraproposta=1, aguardando=1, aceita=1, recusada=1` | `visitas: realizada=3, agendada=2, cancelada=1`

---

### Task 10: Seed captacoes (8)

**Files:** nenhum

- [ ] **Step 1: Inserir 8 captações**
...
```sql
SELECT status, count(*) FROM public.captacoes GROUP BY status ORDER BY status;
```

Esperado: `autorizado=2, em_avaliacao=3, prospectando=2, recusado=1`

---

### Task 11: Seed blog_posts (3)

**Files:** nenhum

- [ ] **Step 1: Inserir 3 posts**
...
(
  ...
  '## Florianópolis tem um bairro para cada estilo de vida
...
**Jurerê Internacional** é sinônimo de sofisticação e alto padrão. Com praias exclusivas, restaurantes badalados e condomínios fechados, é a escolha de quem busca um estilo de vida premium e não se preocupa com o preço por metro quadrado.

**Lagoa da Conceição** atrai um público jovem e criativo. A combinação de natureza exuberante, bares descolados e uma comunidade diversa fazem deste bairro um dos mais cobiçados de Floripa. Ideal para quem valoriza qualidade de vida e movimento.

**Trindade e Itacorubi** são os bairros dos profissionais e universitários. Com fácil acesso à UFSC, ao centro empresarial e ao metrô-cable, oferecem ótimo custo-benefício para quem precisa de praticidade no dia a dia.

**Centro e Estreito** mantêm a conveniência de tudo à mão — comércio, serviços públicos, transporte. São opções sólidas para quem prioriza deslocamento rápido e infraestrutura urbana completa.

## Nossa dica

Visite os bairros em diferentes horários do dia e da semana antes de decidir. A HOME Imob tem corretores especializados em cada região e pode ajudá-lo a encontrar o imóvel certo para o seu perfil.',
  NULL,
  'publicado',
  'Ana Lima',
...
(
  ...
  '## Por que entender o financiamento é tão importante?

Comprar um imóvel é uma das maiores decisões financeiras da vida. Mesmo que você já tenha uma parte do valor, o financiamento imobiliário costuma ser o caminho mais acessível para a realização desse sonho. Conhecer as modalidades disponíveis pode significar uma economia de dezenas de milhares de reais ao longo dos anos.

## SFH x SFI: qual a diferença?

O **Sistema Financeiro de Habitação (SFH)** é voltado para imóveis de até R$ 1,5 milhão e permite o uso do FGTS. As taxas costumam ser mais favoráveis — entre 10% e 12% ao ano com correção pela TR. Já o **Sistema Financeiro Imobiliário (SFI)** cobre imóveis acima desse valor, com taxas negociadas diretamente com o banco, sem limitação de teto.

## Documentos essenciais

Para dar entrada no processo, você precisará de documentos pessoais (RG, CPF, comprovante de renda dos últimos 3 meses), extrato do FGTS (se for usar), e a documentação do imóvel — matrícula atualizada, certidão negativa de débitos e habite-se.

## Dicas práticas

Simule em pelo menos 3 bancos diferentes antes de fechar negócio. O Banco do Brasil, Caixa Econômica Federal e bancos privados frequentemente têm condições distintas para o mesmo perfil de cliente. A HOME Imob possui parceiros financeiros que podem ajudá-lo a conseguir as melhores condições do mercado.',
  NULL,
  'publicado',
  'Marco Vieira',
...
Santa Catarina consolidou-se como um dos mercados imobiliários mais aquecidos do Brasil. A combinação de qualidade de vida, crescimento econômico e migração de profissionais de outros estados criou uma demanda consistente tanto para compra quanto para locação.

## Destaques por cidade

Florianópolis segue valorizada pelo turismo e pelo crescimento do setor de tecnologia. Balneário Camboriú mantém sua reputação de "Dubai brasileira" com lançamentos de alto padrão. Joinville e Blumenau apresentam ótimo custo-benefício para investidores que buscam retorno sólido no mercado de locação.

## O que esperar para 2026

*Este artigo está em elaboração. Em breve, publicaremos a análise completa com dados do mercado.*',
  NULL,
  'rascunho',
  'Ana Lima',
...
```sql
SELECT status, titulo FROM public.blog_posts ORDER BY status;
```

Esperado: 2 publicados, 1 rascunho.
...
- [ ] **Step 1: Contar todos os registros**
...
- [ ] **Step 2: Verificar dashboard KPIs**

```sql
-- Imóveis ativos
SELECT count(*) FROM public.imoveis WHERE status = 'ativo';
-- Esperado: 30

-- Aluguéis a vencer (pendente + atrasado)
SELECT count(*) FROM public.pagamentos WHERE status IN ('pendente', 'atrasado');
-- Esperado: 6 (5 pendentes + 1 atrasado)

-- Leads do mês atual
SELECT count(*) FROM public.leads WHERE created_at >= date_trunc('month', NOW());
-- Esperado: leads com created_at em abril/2026

-- Receita dos últimos 6 meses
SELECT date_trunc('month', created_at) AS mes, sum(valor) AS receita
FROM public.pagamentos WHERE status = 'pago'
GROUP BY mes ORDER BY mes;
-- Esperado: 6 meses com valores crescentes
```

- [ ] **Step 3: Commit de registro**

```bash
git commit --allow-empty -m "data: seed mock data — 13 tabelas, 177 registros"
```
